/**
 * Fix GStreamer to play complete video files without stopping early
 * Preserves working sync while fixing EOS and queue issues
 */

const fs = require('fs');
const path = require('path');

// GStreamer implementation with complete playback fix
const COMPLETE_PLAYBACK_FIX = `
  /**
  ...
  async startGStreamerVideoFileStreaming() {
    ...
    try {
      ...
      // Start GStreamer with proper EOS handling for complete playback
      await this.startCompletePlaybackPipeline(videoFile, width, height, frameRate);
      
      // Mark as using GStreamer
      this.useGStreamer = true;
      
      console.log(\`✅ ViewBot \${this.botId}: GStreamer streaming started for complete playback\`);
  ...
  /**
   * Start GStreamer pipeline with fixes for complete playback
   * Uses rtpbin with proper queue configuration and EOS handling
   */
  async startCompletePlaybackPipeline(videoFile, width, height, frameRate) {
    const { spawn } = require('child_process');
    
    // Pipeline with rtpbin and fixed queue settings for complete playback
    const pipeline = [
      // RTP bin for managing RTP sessions
      'rtpbin', 'name=rtpbin', 'latency=0', 'do-lost=true',
      
      // File source and demuxer
      'filesrc', \`location=\${videoFile}\`,
      '!', 'decodebin', 'name=decoder',
      
      // Video branch with fixed queue settings
      'decoder.',
      '!', 'queue',
        'max-size-time=0',         // No time limit - prevents blocking
        'max-size-buffers=0',      // No buffer count limit
        'max-size-bytes=104857600', // 100MB limit for safety
      ...
      '!', 'udpsink',
        ...
        // Default sync=true for proper timing
        
      // Audio branch with fixed queue settings
      'decoder.',
      '!', 'queue',
        'max-size-time=0',         // No time limit
        'max-size-buffers=0',      // No buffer count limit
        'max-size-bytes=10485760', // 10MB limit
        'leaky=downstream',
      '!', 'audioconvert',
      '!', 'audioresample',
    ...
    console.log(\`🚀 ViewBot \${this.botId}: Launching GStreamer with complete playback support\`);
    console.log(\`   Video: port=\${this.videoRtpPort}, ssrc=\${this.videoSSRC}\`);
    console.log(\`   Audio: port=\${this.audioRtpPort}, ssrc=\${this.audioSSRC}\`);
    console.log(\`   Queue: max-size-time=0 for uninterrupted playback\`);
    
    // Use -e flag to ensure EOS is properly handled
    ...
    this.gstreamerProcess.stderr.on('data', (data) => {
      ...
      if (output.includes('ERROR')) {
        ...
        if (output.includes('Can\\'t record audio fast enough')) {
          console.warn(\`⚠️ ViewBot \${this.botId}: Audio timing warning (non-critical)\`);
        }
        ...
        console.log(\`✅ ViewBot \${this.botId}: Pipeline completed successfully\`);
      }
    });
    ...
    this.gstreamerProcess.on('exit', (code, signal) => {
      ...
      if (eosReceived) {
        console.log(\`   ✅ Video played to completion (EOS received)\`);
      } else if (code === 0) {
        console.log(\`   ✅ Pipeline completed normally\`);
      } else if (code !== 0 && !eosReceived) {
        console.warn(\`   ⚠️ Pipeline exited without EOS (may have been interrupted)\`);
        if (errorBuffer) {
          console.error(\`   Last output:\`, errorBuffer.slice(-500));
        }
      }
      
      this.gstreamerProcess = null;
      
      // If looping is enabled, restart the pipeline
      if (this.config.loop && !this.stopping) {
        console.log(\`🔄 ViewBot \${this.botId}: Restarting video playback (loop enabled)\`);
        setTimeout(() => {
          this.startCompletePlaybackPipeline(videoFile, width, height, frameRate)
    ...
    await new Promise((resolve, reject) => {
      const startTimeout = setTimeout(() => {
        if (!pipelineStarted) {
          const error = new Error('GStreamer pipeline failed to start within 25 seconds');
      ...
      }, 25000); // 25 second timeout for larger files
  ...
  cleanupGStreamerProcesses() {
    ...
    if (this.gstreamerProcess && !this.gstreamerProcess.killed) {
      ...
      setTimeout(() => {
        if (this.gstreamerProcess && !this.gstreamerProcess.killed) {
          console.log(\`⚠️ ViewBot \${this.botId}: Force killing GStreamer process\`);
...
async function applyCompletePlaybackFix() {
  console.log('🔧 Applying GStreamer complete playback fix...');
  ...
  content = beforeMethod + COMPLETE_PLAYBACK_FIX + '\n  ' + afterMethod;
  
  // Write the fixed content
  fs.writeFileSync(filePath, content, 'utf8');
  
  console.log('✅ GStreamer complete playback fix applied!');
  console.log('\n🔑 Key improvements:');
  console.log('1. ✅ Set max-size-time=0 on queues (prevents blocking on 1-second limit)');
  console.log('2. ✅ Set max-size-buffers=0 (removes buffer count restrictions)');
  console.log('3. ✅ Added -e flag for proper EOS handling');
  console.log('4. ✅ Added leaky=downstream to prevent queue overflow');
  console.log('5. ✅ Monitors EOS event for completion detection');
  console.log('6. ✅ Preserves sync=true (default) for proper timing');
  console.log('7. ✅ Added do-lost=true to rtpbin for packet loss handling');
  console.log('8. ✅ Optional loop support for continuous playback');
  console.log('\n⚠️ Restart the server for changes to take effect');
}

applyCompletePlaybackFix().catch(console.error);