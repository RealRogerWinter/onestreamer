...
- **PR 3.3 — `BuffNotifier` for the buff/inventory event cluster (28 sites across 3 events).** Applied to `streamer-buffs-update` (9 sites), `inventory-updated` (9 sites), and `buff-error` (10 sites). This was the structurally messiest of the three: emits split across service code (`BuffDebuffService` ×4, `ItemUseService` ×4, `DrawingService` ×1, `ThrowingService` ×1) AND socket handlers (`BuffHandler` ×11, `StreamHandler` ×2) AND inline route handlers (`routes/items.js` ×3) AND `server/index.js` ×2 — the route/service split was the structural issue called out in the Phase 3 plan. **Three methods on one service** rather than three services, because the buff/inventory cluster is conceptually one feature and splitting would force every consumer to inject three deps. **Three target scopes** the methods serve: `streamerBuffsUpdate({ buffs, toSocket? })` is broadcast OR per-socket (the per-query response variant in `BuffHandler.get-streamer-buffs`); `inventoryUpdated({ toSocketId, action, ... })` is always targeted via `io.to(socketId).emit`; `buffError({ toSocket, error })` is always per-calling-socket. **Action-string pinning** for `inventory-updated` mirrors `StreamNotifier.REASONS`: 6 actions (`purchase`, `sell`, `grant`, `use`, `draw`, `throw`) are pinned in `INVENTORY_ACTIONS` with the same strict-equality size check (deletion caught by `test.each`, addition forced by size pin). The other two events don't have discriminator fields (`{ buffs }` and `{ error }` only), so no pinning needed there. **Construction**: `(io)` only — payload comes from callers. Wired into `BuffDebuffService` via its existing 5th-arg options bag (alongside `itemRepository`), which is identity-pinned in the bootstrap test so the slot can't be repurposed silently. Route handlers read via `req.app.get('buffNotifier')` (added to `app.set` alongside the other route-accessible services). The three method-arg services (`ItemUseService`, `DrawingService`, `ThrowingService`) accept it as a method parameter passed by `routes/items.js`.
...
2. **PR 3.3** — same pattern for the buff/inventory cluster. _Done in `buff-notifier` branch; see "Subsequent applications" above. Implemented as `BuffNotifier` with three methods rather than `BuffService.update()` as the briefing speculated, because the cluster is three distinct events with three target scopes — one method per event was the natural shape once the actual emit sites were mapped. The briefing also mentioned `server/routes/buffs.js` as a source of buff-cluster emits; in practice that file has zero such emits — the real route-side fan-out is `server/routes/items.js` (3 sites) and the rest is split between services + socket handlers + `server/index.js`._
